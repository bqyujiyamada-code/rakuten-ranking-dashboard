...
## Gemini呼び出しのタイムアウト設定・並行化 (300秒予算を使い切らせない)
...
try/catchでは短縮できない**ため、この設計だけでは不十分だった。再発防止として
`generateTrendInsight`(`gemini.ts`)・`fetchDailyTrendSummary`(`trendSummary.ts`)の両方の
`generateContent`呼び出しに`config.httpOptions`で明示的な`timeout`と`retryOptions.attempts`を
設定している。

**2026-08-06・07と、Cron実行時刻を2時間前倒し(対応31.)した後も含め3日連続でAI分析がほぼ
全滅する障害が発生した。** 当初は「7:00 JST前後の米国日中帯にGeminiが混雑する」時間帯要因を
疑い対応31.でCronを5:00 JSTへ前倒ししたが、翌日も全く同じ失敗パターン(和菓子ジャンルのみ
成功)が再現し、`gemini-3-flash-preview`自体が時間帯によらず全般的に高負荷/劣化している
可能性が濃厚と判断した(対応33.)。手動復旧時にタイムアウトを45秒(本番の12秒の3.75倍)に
延ばしても、16ジャンル中3ジャンルしか初回成功せず、全件揃うまでに503/504を挟みながら
計4回のバッチ実行を要した。この実測から、**単に本番のタイムアウト値を伸ばすだけでは
不十分**(そもそも直列実行では300秒予算の中に16ジャンル分の十分な余裕を作れない)と判断し、
`collectAndAnalyze.ts`の処理を2フェーズ構成に再設計した。

- **フェーズ1(ランキング収集、`prepareGenre`)**: 楽天APIのレート制限により従来通り直列
  (ジャンル間1.1秒待機)。この時点ではGemini呼び出しは一切行わない。
- **フェーズ2(Gemini分析、`analyzeGenre`)**: 以前は「ジャンルごとのループの中でランキング
  取得直後に逐次呼び出す」直列実行だったが、`mapWithConcurrency`ヘルパーで
  `GEMINI_ANALYSIS_CONCURRENCY`(8)件ずつ並行実行するよう変更した。16ジャンルなら最大2バッチ。
  **直列実行だと「短いタイムアウトのfail-fastを16回積み上げる」だけで予算の大半を消費し、
  個々の呼び出しに十分な時間を与えられていなかったことが根本原因だったため**、これを解消する
  狙い。
- 気象・トレンドの取得(`getOrFetchWeatherContext`/`getOrFetchTrendContext`)は、以前は
  フェーズ1開始前に直列でawaitしていたが、楽天APIのレート制限とは無関係なためフェーズ1と
  並行して開始するよう変更した(`dailyContextPromise`)。フェーズ1の待ち時間(概算70秒)に
  相乗りさせることで、追加の時間コストをほぼゼロにしている。
- 並行化で生まれた時間的余裕を使い、`generateTrendInsight`側の`GEMINI_HTTP_OPTIONS`を
  `timeout: 12_000`→`30_000`(実測でGeminiの応答が25秒程度かかることがあるため)、
  `retryOptions.attempts`を`1`→`2`(1回だけリトライ)に変更した。`fetchDailyTrendSummary`側の
  `TREND_HTTP_OPTIONS`も`timeout: 20_000`→`35_000`、`attempts: 1`→`2`に変更している。
  **SDK既定のリトライ回数(5回)のまま伸ばすと2026-08-05の障害の再発になるため、上限は
  明示的に2に固定すること。この方針(リトライは最小限・明示的な上限を必ず設定)は崩さないこと。**
- Gemini呼び出しの同時実行数を無制限(16件同時)にしなかったのは、`gemini-3-flash-preview`の
  秒間/分間レート制限がドキュメント化されていない中で、無制限の同時発火がバースト起因の
  429/503をかえって誘発するリスクを避けるための保守的な判断。`GEMINI_ANALYSIS_CONCURRENCY`は
  将来チューニング対象として定数化してある(値を上げる際は、Gemini APIキーのレート制限枠を
  確認してから)。
- **予算計算の考え方**: 300秒予算のうち、フェーズ1(楽天API、概算70秒。トレンド要約はここに
  相乗りするため実質追加コストなし)を差し引いた残り約230秒を、フェーズ2(最大2バッチ ×
  リトライ込みの`generateTrendInsight`呼び出し)で使う想定。タイムアウト値や
  `GEMINI_ANALYSIS_CONCURRENCY`を変更する際は、**「フェーズ2の全バッチが最悪ケース
  (タイムアウト×リトライ回数いっぱいまで待つ)になっても300秒を超えない」ことを確認すること**。
  個々の呼び出しが速く成功する前提の見積もりをしないこと(2026-08-05の障害はまさにその前提が
  崩れて起きた)。
- **本番での実地検証は未実施(対応33.時点)**: 本番DynamoDBに繋いだ状態で
  `collectAndAnalyzeAllGenres`を直接実行すると、本日分のランキングスナップショットが
  2重に作成され`GenreMeta.previousTimestamp`が書き換わって翌日の差分検知の基準がずれてしまう
  ため、この2フェーズ構成の並行化自体を実データで通しテストすることは意図的に見送った
...
### トレンド要約のモデルをGA版へ切り替え (対応40., 2026-08-28)

**ユーザー報告「世間のトレンド要約が取得できていない」を調査。ジャンル分析(`gemini.ts`)は
16/16正常なのに、世間のトレンド要約(`trendSummary.ts`)だけが `collect`・`retry` の両cronで
断続的に失敗していた(8/26・8/28がnull)。** 本番ログでは `504 DEADLINE_EXCEEDED`、ローカル
再現では `503 "This model is currently experiencing high demand"` を即座に返す。原因は
`gemini-3-flash-preview`(preview版)がGAの後継モデル登場後も慢性的に高負荷で、特に
`googleSearch` grounding付きの呼び出しがほぼ通らなくなっていたこと。grounding無しなら通るが
検索結果に基づかないので要約として無意味(「2026年は未来のため予測を含む」と前置きし始める)。

- **対応**: `src/lib/analysis/trendSummary.ts`の`MODEL`を`gemini-3-flash-preview`→
  **`gemini-3.5-flash`**(GA版)に変更。grounding付きで実測4/4成功・18〜27秒・ソース5〜11件と
  安定。`responseSchema`と`googleSearch`が同時利用不可な制約は変わらないため、`gemini.ts`とは
  別のプレーンテキスト呼び出しに分離する設計・`stripMarkdown`適用は維持している。
- `TREND_HTTP_OPTIONS.timeout`を`35_000`→`45_000`に延長(実測で27秒近くかかることがあるため)。
  `retryOptions.attempts`は2のまま(SDK既定5回に戻さない方針は不変)。この呼び出しはフェーズ1
  (楽天API・概算70秒)に相乗りするため、45秒×2でも300秒予算には影響しない。
- **`gemini.ts`(ジャンル分析)のモデルは変更していない。** 報告時点で16/16成功しており、
  構造化出力・16ジャンル並行・300秒予算と絡むため今回のスコープ外とした。将来 `gemini.ts` 側でも
  同種の高負荷障害が再発したら、まず `gemini-3.5-flash`(構造化出力も実測OK)への切り替えを
  検討すること。
- デプロイ後、`/api/cron/retry`を手動で叩けば当日分のトレンド要約を復旧できる(冪等)。

### トレンド要約モデルを floating alias(`gemini-flash-lite-latest`)へ (対応41., 2026-08-31)

**対応40.で切り替えた`gemini-3.5-flash`も、切り替えから3日で対応40.以前とまったく同じ症状を
再発させた。** ユーザー報告「本日の世間のトレンド要約が取得できていません」を調査したところ、
8/29・8/30は取得できていたが8/31分が collect・retry の両cronで失敗し`trend: null`。ジャンル
分析(`gemini.ts`、grounding無し・構造化出力)は正常。ローカル再現で`gemini-3.5-flash`は
`googleSearch` grounding付きの呼び出しが `503 high demand` / `504 DEADLINE_EXCEEDED`、または
30〜65秒かかった末にタイムアウトし、0/2成功。

- **調査手順(再発時はこれを繰り返す)**: 同時刻に複数モデルを grounding付きで並行に叩き、
  その時点で安定しているものを選ぶ。実測結果(2026-08-31 14時JST): `gemini-flash-lite-latest`
  = 4〜6秒・ソース4〜7件で **5/5成功**。`gemini-flash-latest` = 48秒成功 or 503(45秒予算に
  乗らない)。`gemini-3.7-flash` = 504。`gemini-3-flash-preview` = 65秒。`gemini-2.5-flash`
  = 404(新規ユーザー利用不可)。
- **対応**: `src/lib/analysis/trendSummary.ts`の`MODEL`を`gemini-3.5-flash`→
  **`gemini-flash-lite-latest`**に変更。あわせて対応33.以降「preview版・特定GAバージョンの
  ピン留めが高負荷時に軒並み落ちる」ことが3回続いた事実を踏まえ、**あえて floating alias
  (`-latest`)を採用**しGoogle側のルーティングに委ねる方針へ切り替えた。grounding付きの
  3〜5個の箇条書き要約はモデルの生成品質をさほど要求しないため、lite系でも実用上の要約品質に
  問題はないと判断(実測の要約内容も具体的でグラウンディングされている)。
- `TREND_HTTP_OPTIONS.timeout`は`45_000`→`30_000`に短縮(lite系は実測4〜6秒。× 2回で最悪
  60秒、フェーズ1の70秒待ちに収まる)。`retryOptions.attempts`は2のまま(SDK既定5回に
  戻さない方針は不変、対応26.)。
- **`gemini.ts`(ジャンル分析)のモデルは今回も変更していない**(報告時点で正常。構造化出力・
  16ジャンル並行・300秒予算と絡むため別スコープ)。将来 `gemini.ts` 側でも高負荷障害が出たら、
  同じ調査手順で当時安定しているモデル(構造化出力対応のもの)へ差し替えること。
...
**懐疑的レビュー(自己批判)の思考手順(対応39.)。** `buildPrompt`の末尾には「草案を組み立てる→
懐疑的な編集者の視点で弱点(データを超えた推論・因果関係の過度な断定・少ない母数の入れ替わりを
強いトレンド扱いしていないか等)を洗い出す→最終稿に磨き上げる」という思考手順を追加している。
Gemini呼び出しを増やす(下書き→批判→改訂を複数回の呼び出しに分ける)方式ではなく、1回の
呼び出し内でプロンプト指示のみで完結させる方式をユーザーと合意の上で選んだ。理由は
Gemini側の高負荷(トレンド要約の504タイムアウト、後述)がまさに発生していた時期で、
呼び出し回数を増やす変更は300秒のVercel実行時間予算・失敗率の両方を悪化させるリスクがあった
ため。出力はスキーマ通り最終稿のtrendAnalysis/forecastのみで思考過程は出力させない設計のため、
呼び出し回数・タイムアウト・並行数への影響は無い。この思考手順を削除・簡略化する変更を行う際は、
「否定的な論調と一度戦わせた上での分析にしたい」というユーザーの意図を踏まえること。
