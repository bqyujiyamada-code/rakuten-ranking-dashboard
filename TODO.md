...
### 39. AIトレンド分析に懐疑的レビュー(自己批判)の思考手順を追加

ユーザーから「今のAIサマリーは否定的な論調と戦わせたうえでの分析か、そうでなければ追加して
解像度・精度を上げられないか」との提案を受け対応した。

- **確認**: 従来の`generateTrendInsight`(`src/lib/analysis/gemini.ts`)は1回のGemini呼び出しで
  プロンプトのガードレール(物語を作りすぎない・因果関係を捏造しない等)に従って直接
  trendAnalysis/forecastを生成するのみで、生成後に懐疑的な視点でレビュー・書き直させる工程は
  無かった。
- **実装方式の選定**: 「1回の呼び出し内でプロンプトに自己批判の思考手順を指示する」方式と
  「下書き→批判→改訂の複数回Gemini呼び出しに分ける」方式をユーザーに提示し、前者を採用した。
  理由: 後者は1ジャンルあたりのGemini呼び出しが2〜3倍になり、300秒のVercel実行時間予算・
  `GEMINI_ANALYSIS_CONCURRENCY`の再計算が必要になる上、対応時点でまさにGemini側の高負荷
  (トレンド要約が3回連続504タイムアウト)に直面していたため、Gemini呼び出し回数を増やす
  変更はリスクが高いと判断した。
- `buildPrompt`(`gemini.ts`)の末尾に「思考手順(頭の中で行い、出力には含めないこと)」を追加。
  草案を組み立てた後、懐疑的な編集者の視点で「データを超えた推論はないか」「因果関係を過度に
  断定していないか」「母数の少なさによる入れ替わりを強いトレンド扱いしていないか」
  「もっと説得力のある反論・別解釈はないか」を自問して弱点を洗い出し、それを踏まえて
  最終稿に磨き上げるよう指示。出力はスキーマ通り最終稿のtrendAnalysis/forecastのみで、
  思考過程自体は出力させない設計のため、呼び出し回数・タイムアウト・並行数・レスポンス
  スキーマ・呼び出し側コードへの変更は一切無い。
- 動作確認: `.env.local`の実APIキーで実際にGeminiを呼び出し、スキーマ通りの出力(2フィールド
  のみ、思考過程の漏れ出しなし)を確認。うなぎ価格上昇のダミーハイライトに対し、安易に季節行事と
  結びつけず「特定の行事時期ではないことから店舗側の戦略」という代替解釈を採用するなど、
  懐疑的レビューを経た形跡のある出力になっていることを確認した。
