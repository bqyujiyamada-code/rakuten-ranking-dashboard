...
- [x] `iam/dynamodb-policy.json`の内容(`UpdateTable`権限+`GSI2_DailyBundle`のARN)を
  ...
  `npm run db:create-table`を実行し、本番テーブルに`GSI2_DailyBundle`を追加(`ACTIVE`化まで
  数分かかったが完了を確認済み)。`/api/dates`・`/api/daily-context`をローカルから実際に
  叩いて200応答になること、コンソールエラーが出ないことをPlaywrightで確認済み(2026-08-04)。
  ただしこの時点ではまだ`DAY#{date}`レコードが1件も無いため、日付ピッカー・判断材料パネルは
  「データなし」で正しく非表示のまま(想定通り)。**Vercel本番へのデプロイと、次回以降の
  自動収集の実行が別途必要。**
- [ ] 対応22.(気象・トレンド統合)のコードをVercel本番にデプロイし、次回以降の自動収集で
  東京の気象データ・Geminiのトレンド要約が正しく取得・保存され(`DAY#{date}`レコードが
  作られ、日付ピッカーに表示されるようになるか)、AI分析文に因果関係の言及が反映されるかを
  実データで確認
